...
for env in "${ENVS[@]}"; do
  for seed in "${SEEDS[@]}"; do
    ...
    echo "[crossfit] env=${env} seed=${seed} source=${SOURCE} gpu=${gpu}"
    cmd=(python run_crossfit_iql.py
    ...
    CUDA_VISIBLE_DEVICES=${gpu} "${cmd[@]}" > "logs/crossfit_${SOURCE}_${env}_seed${seed}.log" 2>&1 &
    job_id=$((job_id + 1))
